...
def random_portfolio_sequence(
    universe, n_positions: int, n_periods: int, turnover: float, seed: int
) -> list[list]:
    ...
    if not (0 < n_positions <= len(universe)):
        raise ValueError(
            f"n_positions ({n_positions}) deve estar em (0, |universe|={len(universe)}]"
        )
...
def null_distribution(
    statistic, universe, n_positions: int, *, n_samples: int = 1000, seed: int = 0
) -> list[float]:
