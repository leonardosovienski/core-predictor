...
def spearman_block_ci(
    pairs, *, block_length: int = 5, n_boot: int = 10_000, confidence: float = 0.95, seed: int = 42
):
    ...
    lo, hi, _ = bootstrap_ci(
        pairs,
        lambda u: spearman([p[0] for p in u], [p[1] for p in u]),  # pyright: ignore[reportArgumentType]
        scheme="moving",
        block_length=bl,
        n_boot=n_boot,
...
def ci_mean(
    data: list[float], confidence: float = 0.95, n_boot: int = 10_000, seed: int = 42
) -> tuple[float, float]:
