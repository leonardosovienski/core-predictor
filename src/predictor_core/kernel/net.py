...
def download_file(
    url: str, dest: pathlib.Path, timeout: int = 120, retries: int = 3, backoff: float = 5.0
) -> pathlib.Path:
...
def get_impersonating_session(impersonate: str = "chrome"):
    ...
    return _cffi_requests.Session(impersonate=impersonate)  # pyright: ignore[reportArgumentType]
        ...
        async def wrapper(*args, **kwargs):
            ...
            for attempt in range(1, attempts + 1):
                try:
                    ...
                    logger.warning(
                        "%s: transitório (%s); tentativa %d/%d, aguardando %.1fs",
                        fn.__name__,
                        type(exc).__name__,
                        attempt,
                        attempts - 1,
