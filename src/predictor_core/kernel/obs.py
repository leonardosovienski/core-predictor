...
ENVELOPE_KEYS = ("timestamp", "domain", "run_id", "event", "code_version", "metrics", "metadata")
...
def emit_event(
    ...
    *,
    ...
    if nao_numericos:
        raise TypeError(
            f"emit_event: 'metrics' aceita só números; não-numéricos: "
    ...
    nao_finitos = [k for k, v in metrics.items() if isinstance(v, float) and not math.isfinite(v)]
    if nao_finitos:
        raise ValueError(
            f"emit_event: 'metrics' com valor não-finito (NaN/inf): "
    ...
    if timestamp is not None:
        ...
        try:
            ...
            raise ValueError(f"emit_event: 'timestamp' inválido ({timestamp!r}) — {exc}") from exc
    record = {
