...
from predictor_core.data.aggregation import consensus_mean, consensus_median
            ...
            try:
                ...
                if idx > 0:
                    emit_event(
                        ...
                        "data.fallback",
                        metrics={"provider_index": idx},
                        metadata={"symbol": symbol, "interval": interval, "used": provider.name},
                    )
                return points
                ...
                emit_event(
                    self._domain,
                    "data.fallback" if proximo else "data.provider_failed",
                    metrics={"provider_index": idx},
                    metadata={
                        "symbol": symbol,
                        "interval": interval,
...
class AggregationRouter:
    ...
    async def _try(self, provider, symbol, interval, limit):
        ...
        try:
            ...
            emit_event(
                ...
                metrics={},
                metadata={"symbol": symbol, "provider": provider.name, "error": type(exc).__name__},
            )
            return provider.name, None
    ...
    async def fetch_ohlcv(
        ...
        if not survivors:
            ...
            raise DataUnavailableError(f"agregação: nenhuma fonte entregou {symbol} ({interval})")
        fused = _AGG_POLICIES[self._policy](survivors)
        emit_event(
            self._domain,
            "data.aggregated",
            metrics={"n_sources": len(survivors), "n_points": len(fused)},
            metadata={"symbol": symbol, "interval": interval, "policy": self._policy, "used": used},
        )
        return fused
