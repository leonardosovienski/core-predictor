...
class ObservationEnvelope:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            self.lifecycle_state == LifecycleState.OFFICIAL_RESULT_FOUND
            and self.official_result is None
        ):
    ...
    def transition(
        ...
        *,
        ...
        if state == self.lifecycle_state:
            if official_result not in (None, self.official_result) or rejection_reason not in (
                None,
        ...
        if state in _ORDERED:
            expected = (
                _ORDERED[_ORDERED.index(self.lifecycle_state) + 1]
                if self.lifecycle_state in _ORDERED
                and self.lifecycle_state != LifecycleState.COMPLETE
                else None
            )
            if state != expected:
                raise CollectionTransitionError(
                    f"transição inválida: {self.lifecycle_state} -> {state}"
                )
...
class CollectionArchive:
    ...
    def history(self, collection_run_id: str, canonical_event_id: str) -> list[ObservationEnvelope]:
        return [
            ObservationEnvelope.from_dict(row["envelope"])
            for row in self._events()
            if row.get("collection_run_id") == collection_run_id
            and row.get("canonical_event_id") == canonical_event_id
        ]
    ...
    def append(
        ...
        if current is not None:
            if (
                ...
                or envelope.collection_run_id != current.collection_run_id
            ):
                raise CollectionTransitionError(
                    "coleta antiga não pode ser reclassificada como nova"
                )
        ...
        self.store.append(
            {
                "record_type": "collection_transition",
                ...
                "state": envelope.lifecycle_state,
...
def aggregate_funnel(
    ...
    *,
    ...
    for item in envelopes:
        ...
        previous_updated = None if previous is None else previous.updated_at
        if previous_updated is None or previous_updated <= item.updated_at:
            latest[key] = item
