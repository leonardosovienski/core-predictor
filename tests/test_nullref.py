...
def test_sequence_shapes_and_uniqueness():
    seqs = random_portfolio_sequence(range(30), n_positions=10, n_periods=5, turnover=0.2, seed=1)
    assert len(seqs) == 5
...
def test_turnover_is_exactly_realized():
    ...
    seqs = random_portfolio_sequence(range(50), n_positions=10, n_periods=6, turnover=0.2, seed=3)
    for a, b in zip(seqs, seqs[1:]):
        assert len(set(a) & set(b)) == 8


def test_turnover_zero_is_buy_and_hold():
    seqs = random_portfolio_sequence(range(30), n_positions=10, n_periods=4, turnover=0.0, seed=2)
    assert all(set(s) == set(seqs[0]) for s in seqs)  # carteira nunca muda


def test_turnover_full_has_no_overlap():
    seqs = random_portfolio_sequence(range(40), n_positions=10, n_periods=3, turnover=1.0, seed=4)
    for a, b in zip(seqs, seqs[1:]):
...
def test_sequence_rejects_small_universe():
    with pytest.raises(ValueError):
        random_portfolio_sequence(
            range(12), n_positions=10, n_periods=2, turnover=1.0, seed=0
        )  # precisa de 20, tem 12
