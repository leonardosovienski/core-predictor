...
def test_register_and_load(tmp_path):
    ...
    register_trial("v1", params={"h": 7}, sharpe=0.1, path=p, now="2026-07-03T00:00:00Z", **_NOGATE)
    register_trial(
        "v2", params={"h": 30}, sharpe=0.2, path=p, now="2026-07-03T00:01:00Z", **_NOGATE
    )
...
def test_extra_desconhecido_e_rejeitado_antes_de_persistir(tmp_path):
    with pytest.raises(ValueError, match="campos extras"):
        register_trial(
            "t-a", params={"h": 7}, path=tmp_path / "trials.json", featuers_used=["typo"], **_NOGATE
        )
...
def test_dsr_decreases_with_more_trials():
    ...
    dsr_many = deflated_sharpe_ratio(_RETURNS, [0.1, 0.12, 0.2, -0.1, 0.3, 0.25, -0.2, 0.4])
...
def test_validate_trials_rejeita_sharpe_bool_e_metric_invalida():
    base = {"name": "x", "registered_at": "2026-01-01T00:00:00Z", "params": {"a": 1}, "notes": ""}
...
def test_entrada_legada_malformada_nao_bloqueia_silenciosamente_trial_nova_valida(tmp_path):
    ...
    p = tmp_path / "trials.json"
    p.write_text(
        '[{"name": "legada sem underscore e com espaco", "registered_at": "x", '
        '"params": {}, "sharpe": null, "notes": ""}]',
        encoding="utf-8",
    )
