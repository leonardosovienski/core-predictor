...
def test_block_bootstrap_brackets_mean_and_is_finite():
    ...
    lo, hi, dist = stats.block_bootstrap_ci(series, lambda u: sum(u) / len(u),
...
def test_block_bootstrap_drops_nonfinite_resamples():
    ...
    lo, hi, dist = stats.block_bootstrap_ci(series, lambda u: float("inf"),
                                            block_length=10, n_boot=200, seed=3)
